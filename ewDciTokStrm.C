...
{
    // if savewrd has processed(or some other case), let nextwrd to be savedwrd(to be processed)
    // and set nextwrd to next wrd in stream
    if( !savedWrd_.length() )
    {
	savedWrd_ = nextWrd_;
	//if( !istr_ )
...
/*** Execution reaches this point iff. a new Wrd has just "come on deck." ***/
    ...
    // wul: pass through the <COMMENT> part 
    ...
    // wul: strip each strange setence. (flush_to_sentence not know it)  
...
{   // wul: 可能的分离符号。但英语里面不一定。比如句号，也可能是小数点
...
/***  Move the puncIndex forward to skip over puncChars
    ...
    // wul: 跳过这些数字中的分离符号，直到转到puncIndex
    ...
    {                                                   //eg   11:45  (time)
        // wul: 构建新的字符串，构造函数为 XXX（参考字符串，起点，长度）  
	...
	if(nextp >= len2) nextp = -1;
        // puncIndex设为-1,或者加上移量（转为全局的seq的偏移量）
	if( nextp == -1)  puncIndex = -1;
        else  puncIndex += ( nextp + 1 );
    }

    // wul: 找到的是句点,继续处理其特殊情况
    while( puncIndex != -1 
	&& seq[ puncIndex ] == '.'
    ...
    // wul: 这里处理由撇号的，且在开头的。（上面一段代码还有‘联字符’）
...
/** In the raw text, the closing dot of a "stateLike" abbreviation may serve
    ...
    // 对缩写（如州名）的点，如果在句子末尾要添加一个点（除非已经有其它的符号）
    ...
    {                                               // no splitting puncChars.
        //词中间没有作为分离符的特殊符号则，返回全词
        //本词没有剩余的（要处理的）部分了
	savedWrd_ = "";
        return seq;
    }
    //词中间有作为分离符的特殊符号（puncIndex作为它的下标）
    if( puncIndex > 0 )
	...
	{                             // trailing-or-internal-apostrophe cases
		...
		if( length > 4 )
		{
                    //savedWrd_保留后半截，返回前面的词，如Can, can, will, Will, IS（特殊）
		...
		{   savedWrd_ = seq.substr( puncIndex-1,length-puncIndex+1 );
		    return seq.substr( 0, puncIndex-1 );
                ...
                // wul: 这里处理puncIndex为1的情况，其实就是返回前3个字符，其余的存在savedWrd_中。前3个字符一定为n't（整体作为一个词）
		...
		return seq.substr( 0,5 );
                                                                 ...
                                                                 // page SEVEN
        //wul: 缺省的处理方法，按照分割符号，前面的一部分返回（不包括分割符号），余下的存为saveWrd_
        //wul: 分割符号在开头 ---NOTE1
	savedWrd_ =
...
**   happen to mean close-quote, and that's how this rule treats them.    ***/
    // wul: 处理puncIndex为0的情况。很多时候是由于前面操作的结果（见NOTE1）
    if( seq[0] == '\"' )
    {
	...
	if( seq == "'cause" )               //eg   ". . . just 'cause I care!"
	{
	    ...
	    return seq;
								...
								// page EIGHT
    // 如果不是前面处理的情况，且本分离符号后面接着同样的分离符号，把这些所有的连续分离符号作为一个词
