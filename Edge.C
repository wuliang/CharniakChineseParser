...
{
  // wul: 从Term创建的Edge。这可以看作原创形Edge，除了lhs设置为Term，其他的都不初始化
  //numEdges++;
...
{
  //numEdges++;
  // wul: 如果start和loc没有初始化，按照item的值初始化
  if(start_ == -1)
    start_ = itm.start();
  if(loc_ == -1) loc_ = itm.finish();
  // wul: 如果已经完成，loc必须到结束位置.
  // 如果未完成，start必须到开始位置 （这说明start和Loc都可以变动？）
...
: lhs_( itm.term() ), loc_( itm.finish() ),  finishedParent_( &itm ),
  ...
  rightMerit_(1)
{
  // wul: 从Item创建Edge.该item将设置为finishedParent，而不是内部的item_
  //numEdges++;
}
